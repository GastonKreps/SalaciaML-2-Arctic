...
# gradients or spikes (Temp_Trad_QF codes 2 or 4).
...
# Remove profiles with an excessive number of flagged points (> 20)
trad_counts = data2_4[data2_4['Temp_Trad_QF'].isin([2, 4])].groupby('Prof_no').size()
...
bad_data = data2_4_filtered[data2_4_filtered['Temp_Trad_QF'] != 0].copy()
bad_data['Temp_Trad_QF'].replace([2, 4], 1, inplace=True)
...
scaler_path = os.path.join(OUTPUT_DIR, 'scaler_temperature_model.pkl')
...
checkpoint_filepath = os.path.join(OUTPUT_DIR, "best_temperature_model.keras")
...
plt.title('Model Loss Over Epochs - Temperature')
plt.ylabel('Loss')
plt.xlabel('Epoch')
plt.legend()
plt.grid(True)
...
plt.plot(fpr, tpr, marker='.', label='Validation')
plt.scatter(fpr[np.argmax(gmeans)], tpr[np.argmax(gmeans)], marker='o', color='black', s=100, zorder=5, 
            label=f'Best Threshold = {best_threshold:.4f}')
plt.title('Receiver Operating Characteristic (ROC) Curve - Temperature')
...
cm_trad = confusion_matrix(test_data[target], test_data['Temp_Trad_QF'])

# Plot confusion matrices
fig, axes = plt.subplots(1, 2, figsize=(16, 6))
sns.heatmap(cm_ml_tqf, annot=True, fmt='d', cmap='Blues', ax=axes[0])
axes[0].set_title('Confusion Matrix: ML + Traditional Filter - Temperature')
axes[0].set_xlabel('Predicted Label')
axes[0].set_ylabel('True Label')

sns.heatmap(cm_trad, annot=True, fmt='d', cmap='Greens', ax=axes[1])
axes[1].set_title('Confusion Matrix: Traditional Method Only - Temperature')
